...
See also: [SECURITY.md](../../SECURITY.md) · [AI-GOVERNANCE.md](../design/AI-GOVERNANCE.md)
§5/§8 · [DATA-GOVERNANCE.md](../design/DATA-GOVERNANCE.md) §2/§7/§9 ·
[CI-CD.md](./CI-CD.md) §3–5 · [DELIVERY-MODEL.md](./DELIVERY-MODEL.md) (tenancy) ·
[ARCHITECTURE.md](../design/ARCHITECTURE.md) §2 · [DECISIONS.md](../project/DECISIONS.md)
(9/10/13/17).
...
| **B4** | backend ↔ Vertex             | **yes**            | **confidential-text egress** to the bank's own GCP Vertex — the high-stakes boundary (open: DECISIONS 10/17)                                       |
...
## 4. ⚠️ STRIDE — threats → controls → residual
...
|                     | confidential text reaching Vertex models                            | model-egress **gate** (AI-GOV §7); the bank's own Vertex + VPC-SC/CMEK                                                                       | **OPEN** — DECISIONS 10/17 (the headline risk)                             |
...
|                     | webhook `endpoint_url` aimed at an **internal** address (SSRF)      | HMAC-signed delivery; admin-only subscription (DATA-MODEL §10)                                                                               | **egress allowlist / URL validation not yet specified — build-phase (§6)** |
...
## 5. 🗺️ Top residual risks (watchlist)

1. **Confidential text → Vertex egress — OPEN.** Even as the **bank's own** GCP Vertex (its
   terms, region, VPC-SC/CMEK), data still leaves the application to a managed AI service; it
   gates parse/embed/judge/serve **and** on-demand translation. The per-enterprise model means
   **the bank owns this call** — resolve DECISIONS **10** (data terms) and **17** (residency) before
   processing confidential tiers; the "no" branch forces the self-hosted AI stack.
...
- **Webhook egress allowlist / `endpoint_url` validation** to prevent SSRF (the subscription
  schema is DATA-MODEL §10; the egress restriction is unspecified in the design).
