...
| M3-4 | **Method-B retrieval: embed→ScaNN ANN (reuse→adapt):** embed a changed control chunk (`FACT_VERIFICATION`) + ANN vs the **target law corpus** | given a control chunk, returns top-N nearest law sections from the right jurisdiction corpus (Local→host, Group→home); Mode-B fake | DATA-MODEL §5 · DEC 1       | M3-3       | M    | Medium | Med  |
